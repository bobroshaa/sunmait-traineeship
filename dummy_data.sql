...
INSERT INTO category (name, parent_category_id) 
	SELECT 'Mini Dresses',id FROM category WHERE name='Dresses & Jumpsuits';

-- FILLING SECTION_CATEGORY TABLE
INSERT INTO section_category (section_id, category_id)
	SELECT (SELECT id FROM section WHERE name='Men''s'),id FROM category WHERE name='Shirts';
INSERT INTO section_category (section_id, category_id)
	SELECT (SELECT id FROM section WHERE name='Women''s'), id FROM category WHERE name='Dresses & Jumpsuits';
INSERT INTO section_category (section_id, category_id)
	SELECT (SELECT id FROM section WHERE name='Women''s'), id FROM category WHERE name='Mini Dresses';
...
INSERT INTO product (name, description, price, sex, brand_id, add_date, category_id, section_id, quantity, image)
	SELECT 'GUCCI Daisy Dotty Mini Dress', 
		...
		now(),
		id,
		(SELECT id FROM section WHERE name='Women''s'),
		100,
...
INSERT INTO product (name, description, price, sex, brand_id, add_date, category_id, section_id, quantity, image)
	SELECT 'Tommy Hulfiger Stone Green Utility Shirt', 
		...
		65.00,
		'Male',
		(SELECT id FROM brand WHERE name='Tommy Hilfiger'),
		now(),
		id,
		(SELECT id FROM section WHERE name='Men''s'),
		322,
...
INSERT INTO product (name, description, price, sex, brand_id, add_date, category_id, section_id, quantity, image)
	SELECT 'Adidas Green Cactus Short Sleeve Shirt', 
		...
		39.00,
		'Male',
		(SELECT id FROM brand WHERE name='Adidas'),
		now(),
		id,
		(SELECT id FROM section WHERE name='Men''s'),
		60,
...
INSERT INTO user_account (email, password, role, first_name, last_name) VALUES
	('bobbyjhon@gmail.com',
...
INSERT INTO user_account (phone, email, password, role, first_name, last_name) VALUES
	('+375336441656',
	 'maryjane@gmail.com',
	 '65e84be33532fb784c48129675f9eff3a682b27168c0ea744b2cf58ee02337c5',
	 'Customer',
...
INSERT INTO address (user_id,country, district, city, postcode, address_line_1, address_line_2) 
	SELECT 
		id,
	 	'Belarus',
	 	'Gomelskaya oblast',
	 	...
	 	'247760',
	 	'bulvar Junosti',
	 	'dom 120' FROM user_account WHERE email='maryjane@gmail.com';

-- FILLING REVIEW TABLE
INSERT INTO review (product_id, user_id, review_title, review, rating, add_date)
	SELECT 
		id, 
		(SELECT id FROM user_account WHERE email='maryjane@gmail.com'),
		'Perfect dress',
	...
	FROM 
		product 
	WHERE 
		name='GUCCI Daisy Dotty Mini Dress';

-- FILLING CUSTOMER_ORDER TABLE
INSERT INTO customer_order (user_id, current_status, order_date)
	SELECT id, 'In review', now() FROM user_account WHERE email='maryjane@gmail.com';

UPDATE customer_order 
	SET current_status='In delivery'
	FROM customer_order AS c LEFT JOIN user_account AS u ON c.user_id=u.id
...
INSERT INTO order_product (product_id, order_id, quantity, price)
	SELECT 
		id,
		(SELECT customer_order.id FROM customer_order LEFT JOIN user_account AS user_acc ON user_id = user_acc.id AND user_acc.email='maryjane@gmail.com'),
		1,
		price
	FROM 
		product 
